...
function isFootWalkable(mask, position) {
  return FOOT_PROBES.every(([offsetX, offsetY]) => {
...
const output = `// Authored overworld terrain layer.\n` +
  `// A set bit means the corresponding terrain cell can support the player's feet.\n` +
  `export const TERRAIN_GRID_SIZE = { width: ${MASK_WIDTH}, height: ${MASK_HEIGHT} } as const;\n\n` +
  `export const WALKABLE_TERRAIN_BITS = ${JSON.stringify(Object.fromEntries(entries.map(({ id, bits }) => [id, bits])), null, 2)} as const;\n`;
await fs.writeFile(path.join(ROOT, "app/collision-mask-data.ts"), output);
console.log(`Generated ${entries.length} authored terrain layers at ${MASK_WIDTH}x${MASK_HEIGHT}.`);
